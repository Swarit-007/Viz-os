...
class VizOS {
    ...
    async simulateCPUScheduling() {
        try {
            ...
            if (this.currentAlgorithm === 'roundrobin' && !(quantum >= 1)) {
                throw new Error('Time quantum must be at least 1');
            }

            let result;
            try {
                result = await postJSON(`/api/scheduling/${this.currentAlgorithm}`, {
                    processes,
                    time_quantum: quantum
                });
            } catch (error) {
                // Only fall back to the in-browser engines when the backend is unreachable
                // (e.g. static hosting); validation errors must reach the user.
                const local = { fcfs: FCFS, sjf: SJF, priority: Priority };
                if (!(error instanceof TypeError)) throw error;
                if (this.currentAlgorithm === 'roundrobin') {
                    result = RoundRobin.simulate(processes, quantum);
                } else if (local[this.currentAlgorithm]) {
                    result = local[this.currentAlgorithm].simulate(processes);
                } else {
                    throw new Error('Backend unavailable: this algorithm needs the server');
                }
            }
    ...
    async simulateBankers() {
        try {
            ...
            if (advancedOpen) {
                ...
                result = await postJSON('/api/bankers', {
    ...
    async simulateDeadlock() {
        try {
            ...
            if (advancedOpen) {
                ...
                result = await postJSON('/api/deadlock', {
                    num_processes: numProcesses,
                    num_resources: numResources,
                    allocation,
                    request,
    ...
    async simulatePageReplacement() {
        try {
            ...
            const result = await postJSON('/api/page-replacement', {
                algorithm: algorithm,
                frames: frames,
    ...
    async simulateMemoryAllocation() {
        try {
            ...
            const result = await postJSON('/api/memory-allocation', {
                strategy: algorithm,
                blocks: blocks,
                processes: processes
            });
