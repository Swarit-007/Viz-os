...
class Deadlock {
    static simulate(numProcesses, numResources) {
        ...
        const waitForGraph = this.generateWaitForGraph(numProcesses, allocation, request, available);
    ...
    static generateWaitForGraph(numProcesses, allocation, request, available) {
        const processes = [];
        const edges = [];
        ...
        const deadlockResult = this.detectDeadlock(allocation, request, 
            available, numProcesses, allocation[0].length);
